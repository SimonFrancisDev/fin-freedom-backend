...
const IMMEDIATE_PASS_DEBOUNCE_MS = 150;
...
export async function runIndexerPass() {
  ...
  const [liveTailSettled, orderedSettled] = await Promise.allSettled([
    runLiveTailSync({
      chainId: context.chainId,
      latestBlock: context.latestBlock,
      targets: context.targets,
    }),
    runIndexerCycle(context),
  ]);

  const liveTail =
    liveTailSettled.status === 'fulfilled'
      ? liveTailSettled.value
      : {
          enabled: LIVE_TAIL_ENABLED,
          skipped: false,
          windowBlocks: LIVE_TAIL_WINDOW_BLOCKS,
          results: [],
          error: buildErrorMessage(liveTailSettled.reason),
        };

  const ordered =
    orderedSettled.status === 'fulfilled'
      ? orderedSettled.value
      : {
          latestBlock: context.latestBlock,
          safeBlock: context.safeBlock,
          results: context.targets.map((target) => ({
            key: target.key,
            status: 'error',
            processed: false,
            safeBlock: context.safeBlock,
            lastProcessedBlock: 0,
            lagBlocks: 0,
            error: buildErrorMessage(orderedSettled.reason),
          })),
        };
...
//     0,
//     Number(safeBlock) - Number(lastProcessedBlock || 0)
//   );
...
//       lagBlocks: Math.max(
//         0,
//         safeBlock - Number(state.lastProcessedBlock || 0)
//       ),
//     };
//   }
...
//       } catch (err) {
//         console.error('[INDEXER_PASS_ERROR]', buildErrorMessage(err));

//         if (isRateLimitError(err) || isOutOfCreditsError(err)) {
//           await sleep(20000);
//         }
//       }
...
//       if (isRateLimitError(error) || isOutOfCreditsError(error)) {
//         const cooldownMs = isOutOfCreditsError(error)
//           ? Math.max(15000, env.RPC_OUT_OF_CREDITS_COOLDOWN_MS)
//           : Math.min(1500 * attempt, 6000);
...
//   console.log('[SAVED_REGISTRATION_EVENT]', {
//     txHash: toLower(log.transactionHash),
//     logIndex: log.index,
//     eventName: parsed.name,
//     user: toLower(args.user || ''),
