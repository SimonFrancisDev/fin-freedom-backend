...
async function isWalletRegisteredIndexed(walletAddress) {
  const cleanWallet = normalizeWallet(walletAddress)

  if (!cleanWallet || cleanWallet === ethers.ZeroAddress.toLowerCase()) {
    return true
  }
...
async function resolveReferralToWallet(ref) {
  ...
  if (ethers.isAddress(cleanRef)) {
    const referrerWallet = cleanRef.toLowerCase()
    const isRegistered = await isWalletRegisteredIndexed(referrerWallet)
    ...
    return {
      referrerCode: codeRecord?.shortCode || referrerWallet,
      referrerWallet,
    }
  }
  ...
  const isRegistered = await isWalletRegisteredIndexed(record.walletAddress)
