...
function mergeGeneratedAndWalletByLevel(generatedReceipts = [], walletReceipts = [], escrowByLevel = new Map()) {
  const generatedByLevel = groupReceiptMoneyByLevel(generatedReceipts, escrowByLevel)
  const walletByLevel = groupReceiptMoneyByLevel(walletReceipts, escrowByLevel)

  const levels = new Set([
    ...generatedByLevel.map((item) => item.level),
    ...walletByLevel.map((item) => item.level),
  ])

  return Array.from(levels)
    .sort((a, b) => a - b)
    .map((level) => {
      const generated = generatedByLevel.find((item) => item.level === level)
      const wallet = walletByLevel.find((item) => item.level === level)
      const base = generated || wallet || {}

      return {
        ...base,
        level,
        orbitType: levelToOrbitType[level] || base.orbitType || '',
        ...
        receiptEscrowLocked:
          ...
          wallet?.receiptEscrowLocked ||
          '0.00',

        escrowLockedLifetime:
          generated?.escrowLockedLifetime ||
          wallet?.escrowLockedLifetime ||
          '0.00',

        autoUpgradeUsed:
          generated?.autoUpgradeUsed ||
          wallet?.autoUpgradeUsed ||
          '0.00',

        escrowReleasedToUser:
          generated?.escrowReleasedToUser ||
          wallet?.escrowReleasedToUser ||
          '0.00',
...
export const fetchUserGlobalSummary = safeApiResponse(async function(address) {
  ...
  const earningsSummary = summarizeReceiptsForViewer(walletReceipts, normalizedAddress);
  ...
  return {
    ...
    earnings: {
      ...
      escrowLockedLifetime: formatUsdt(
        maxBigInt(
          escrowMetrics.lockedLifetimeRaw,
          walletReceiptTotals.receiptEscrowLockedRaw
        )
      ),
      autoUpgradeUsed: formatUsdt(escrowMetrics.usedForUpgradeRaw),
      escrowReleasedToUser: formatUsdt(escrowMetrics.releasedToUserRaw),
