...
async function trySafe(label, fn, fallback) {
  try {
    return await fn();
  } catch (error) {
    console.error(`[ORBIT_LEVEL_ENRICHER_ERROR] ${label}`, error);
    return fallback;
  }
}
...
export async function enrichOrbitLevelSnapshot(address, level) {
  ...
  const { orbitContract } = pickOrbitContract(contracts, level);
  const { registration } = contracts;

  // These fields remain DB/snapshot driven and should not be recomputed from chain.
  let isLevelActive = Boolean(snapshot.isLevelActive ?? false);
  ...
  const [
    ...
    trySafe(
      `getLockedForNextLevel failed for ${normalizedAddress} level ${level}`,
      () => getLockedForNextLevel(contracts, normalizedAddress, level),
      null
    ),
  ]);

  isLevelActive = Boolean(levelActiveRaw);

  if (orbitSummaryRaw) {
    orbitSummary = {
      ...orbitSummary,

      // enrich only the fields that are safe to read from chain at this layer
...
//   // Start from snapshot truth.
//   // These are the cycle-structure fields and MUST remain DB-driven:
