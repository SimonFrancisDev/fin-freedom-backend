...
function buildRuleViewFromIndexedEvents(eventsForPosition) {
...
function getLastResetEvent(events) {
  const resets = getResetEvents(events);
...
function getCompletedCycleCount(events) {
  const resets = getResetEvents(events);
  if (!resets.length) return 0;

  return Math.max(...resets.map((event) => Number(event.cycleNumber || 0)));
}
...
function inferMirrorActivation(eventsForPosition = []) {
  const spilloverEvent = eventsForPosition.find(
    (event) => event.eventName === 'SpilloverPaid'
  );
  return Boolean(spilloverEvent);
}

function hasFilledSnapshot(snapshot) {
...
function preserveExistingPositionSnapshot(existingSnapshot, currentCycleNumber) {
  return {
    occupant: existingSnapshot.occupant || null,
    amount: existingSnapshot.amount || '0.0',
    timestamp: Number(existingSnapshot.timestamp || 0),
    activationId: Number(existingSnapshot.activationId || 0),
    activationCycleNumber:
      Number(existingSnapshot.activationCycleNumber || 0) || currentCycleNumber,
    isMirrorActivation: Boolean(existingSnapshot.isMirrorActivation || false),
    truthLabel: existingSnapshot.truthLabel || 'UNKNOWN',
    indexedEventCount: Number(existingSnapshot.indexedEventCount || 0),
    indexedReceiptCount: Number(existingSnapshot.indexedReceiptCount || 0),
    receiptTotals: existingSnapshot.receiptTotals || buildEmptyReceiptTotals(),
    viewerReceiptBreakdown:
      existingSnapshot.viewerReceiptBreakdown || buildEmptyViewerBreakdown(),
    indexedReceipts: existingSnapshot.indexedReceipts || [],
    indexedEvents: existingSnapshot.indexedEvents || [],
    ruleView: existingSnapshot.ruleView || null,
  };
}
...
export async function buildOrbitPositionSnapshot(address, level, position, options = {}) {
  ...
  const [existingSnapshot, allEvents, allReceipts] = await Promise.all([
    OrbitPositionSnapshot.findOne({
  ...
  const receiptsForPosition = sortByChainPoint(
    filteredReceipts.filter(
      (receipt) => Number(receipt.sourcePosition || 0) === position
    )
  );
  ...
  eventsForPosition.length > 0
    ? eventsForPosition[eventsForPosition.length - 1]
    : null;
  ...
  const ruleView = buildRuleViewFromIndexedEvents(eventsForPosition);
  const activationId = findActivationIdFromReceipts(receiptsForPosition);
  const isMirrorActivation = inferMirrorActivation(eventsForPosition);
  ...
  const update = {
    ...
    occupant: rebuiltSnapshot.occupant,
    amount: rebuiltSnapshot.amount,
    timestamp: rebuiltSnapshot.timestamp,

    activationId: rebuiltSnapshot.activationId,
    activationCycleNumber: rebuiltSnapshot.activationCycleNumber,
    isMirrorActivation: rebuiltSnapshot.isMirrorActivation,

    truthLabel: rebuiltSnapshot.truthLabel,
    indexedEventCount: rebuiltSnapshot.indexedEventCount,
    indexedReceiptCount: rebuiltSnapshot.indexedReceiptCount,

    receiptTotals: rebuiltSnapshot.receiptTotals,
    viewerReceiptBreakdown: rebuiltSnapshot.viewerReceiptBreakdown,

    indexedReceipts: rebuiltSnapshot.indexedReceipts,
    indexedEvents: rebuiltSnapshot.indexedEvents,
    ruleView: rebuiltSnapshot.ruleView,
    ...
    metadata: {
      ...
      completeness: {
        ...
        ruleViewReady: !!rebuiltSnapshot.ruleView,
      },
      cycleDerivation: {
        currentCycleNumber,
  ...
  const snapshot = await OrbitPositionSnapshot.findOneAndUpdate(
    { address: normalizedAddress, level, position },
    { $set: update },
    {
      upsert: true,
  ...
  logDebug('[ORBIT_POSITION_SNAPSHOT_BUILT]', {
    address: normalizedAddress,
    level,
    position,
    orbitType,
    occupant: update.occupant,
    activationId: update.activationId,
    activationCycleNumber: update.activationCycleNumber,
    isMirrorActivation: update.isMirrorActivation,
    indexedEventCount: update.indexedEventCount,
    indexedReceiptCount: update.indexedReceiptCount,
    preservedFromExisting,
  });
